...
def trim_video_wrapper(file, tmp_dir, output_dir, pad):
    """
    Wrapper for parallel processing
    """
    video_file, elan_file = file[0], file[1]
    video_id = os.path.basename(video_file).split('.mp4')[0]
...
def main(video_dir, elan_dir, output_dir,
         padding_format=0.1, num_jobs=1, tmp_dir='/tmp/video_trim'):
    ...
    if num_jobs==1:
        ...
        for idx, file in enumerate(files):
            status_lst.append(trim_video_wrapper(file, tmp_dir, 
                                        output_dir, padding_format))
    else:
        status_lst = Parallel(n_jobs=num_jobs)(delayed(trim_video_wrapper)(
                            file, tmp_dir, 
                            output_dir, padding_format) for idx, file in files)
